...
/**
  ...
  * @retval None
  */
void HAL_GPIO_EXTI_IRQHandler(uint16_t GPIO_Pin) // A ISR to deal with external interrupts
{
  /* EXTI line interrupt detected */
  if (__HAL_GPIO_EXTI_GET_IT(GPIO_Pin) != RESET) // checks if pin is active
  {
    __HAL_GPIO_EXTI_CLEAR_IT(GPIO_Pin); // clears the interrupt
